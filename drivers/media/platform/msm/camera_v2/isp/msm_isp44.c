...
#undef CDBG
#define CDBG(fmt, args...) PRINTKL(fmt, ##args)
//Charm end
...
//	if (vfe_dev->fs_vfe) {
	if (!IS_ERR_OR_NULL(vfe_dev->fs_vfe)) {
	...
	if (rc < 0) {
		PRINTKL2("!!! irq request failed");
		pr_err("%s: irq request failed\n", __func__);
		goto irq_req_failed;
	}
	PRINTKL("irq request success %d",vfe_dev->vfe_irq->start);
	return rc;
irq_req_failed:
...
static int msm_vfe44_get_platform_data(struct vfe_device *vfe_dev)
{
	...
	vfe_dev->vfe_irq = platform_get_resource_byname(vfe_dev->pdev,
		IORESOURCE_IRQ, "vfe");
	PRINTKL2("irq number=%d",vfe_dev->vfe_irq->start);
	if (!vfe_dev->vfe_irq) {
...
//		if (!vfe_dev->iommu_secure_ctx[0]) {
		if (IS_ERR_OR_NULL(vfe_dev->iommu_secure_ctx[0])) {
