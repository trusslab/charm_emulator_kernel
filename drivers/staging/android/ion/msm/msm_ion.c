...
//Charm adding temp 
//FIXME
struct ion_heap *ion_secure_cma_heap_create(struct ion_platform_heap * temp_in){
	struct ion_heap* temp_out;
	temp_out= kmalloc( sizeof(temp_out),GFP_KERNEL );
	return temp_out;

}
void ion_secure_cma_heap_destroy(struct ion_heap * temp_in){
	return;
}
