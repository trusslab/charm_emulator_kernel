...
static void *kgsl_iommu_create_pagetable(void)
{
	...
	if (domain_num >= 0) {
		...
		if (iommu_pt->domain) {
			//Charm
			PRINTKL("[5]: FIXED (comment next two lines)\n");
	...
	for (i = 0; i < iommu->unit_count; i++) {
		...
		for (j = 0; j < iommu_unit->dev_count; j++) {
			...
			if (!iommu_unit->dev[j].attached) {
				...
				//Charm
				PRINTKL("[1.1]: drvdata = %#lx\n", drvdata);
				if (!drvdata) {
					//Charm start
					PRINTKL("[1.2]: iommu_unit = %#lx\n", iommu_unit);
					PRINTKL("[1.3]: iommu_unit->dev[j].dev = %#lx\n", iommu_unit->dev[j].dev);
					PRINTKL("[1.4]: iommu = %#lx (FIXME)\n", iommu);
					//Charm end
					//Charm start: FIXME ...dev is a remote ptr
					////drvdata = dev_get_drvdata(
					////iommu_unit->dev[j].dev->parent);
					////iommu_unit->clks[0] = drvdata->pclk;
					////iommu_unit->clks[1] = drvdata->clk;
					////iommu_unit->clks[2] = drvdata->aclk;
					iommu_unit->clks[0] = NULL;
					iommu_unit->clks[1] = NULL;
					iommu_unit->clks[2] = NULL;
					//Charm end
					iommu_unit->clks[3] =
							iommu->gtcu_iface_clk;
				}
				//Charm
				PRINTKL("[2]: FIXED: previous cond.\n");
			}
		}
	}
done:
