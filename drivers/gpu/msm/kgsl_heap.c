...
//Charm start: FIXME: implement the next two based on kgsl_heap_shrink()
static unsigned long kgsl_heap_count_objects(struct shrinker *shrinker,
...
static unsigned long kgsl_heap_scan_objects(struct shrinker *shrinker,
			      struct shrink_control *sc)
{
	BUG();
	return 0;
...
int kgsl_heap_init(void)
{
	...
	kgsl_heap.shrinker.count_objects = kgsl_heap_count_objects;
	kgsl_heap.shrinker.scan_objects = kgsl_heap_scan_objects;
	//Charm end
