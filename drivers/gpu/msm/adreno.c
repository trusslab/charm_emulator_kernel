...
void _soft_reset(struct adreno_device *adreno_dev)
{
	...
	if (gpudev->regulator_enable_original)
		gpudev->regulator_enable_original(adreno_dev);
	//Charm end	
}
	...
	struct kgsl_device_platform_data *pdata)
{
	...
	//Charm start: FIXME
	adreno_dev->dev.mmu.secured = false;
	PRINTKL("[1]\n");
	//Charm end
		...
		unsigned int mem_len)
{
	...
	//Charm
	PRINTKL("[1]: reg = %#lx, value = %#lx\n", reg, value);
	//Charm start
	//if (reg == 0xffffc900000806c0)
	//	dump_stack();
	//Charm end
...
static void adreno_regulator_disable(struct kgsl_device *device)
{
	...
	if (gpudev->regulator_disable_original &&
		test_bit(ADRENO_DEVICE_GPU_REGULATOR_ENABLED,
			&adreno_dev->priv)) {
		////gpudev->regulator_disable(adreno_dev);
		gpudev->regulator_disable_original(adreno_dev);
	//Charm end
...
static const struct kgsl_functable adreno_functable = {
	...
	.regulator_enable_original = adreno_regulator_enable,
	.is_hw_collapsible = adreno_is_hw_collapsible,
	////.regulator_disable = adreno_regulator_disable,
	.regulator_disable_original = adreno_regulator_disable,
	//Charm end
